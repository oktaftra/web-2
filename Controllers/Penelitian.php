...
class Penelitian
{
    ...
    {
        $stmt = $this->pdo->query("
            SELECT p.*, bi.nama AS nama_bidang_ilmu
            FROM penelitian p
    ...
    {
        $sql = "UPDATE penelitian
                SET judul = :judul, mulai = :mulai, akhir = :akhir,
                    tahun_ajaran = :tahun_ajaran, bidang_ilmu_id = :bidang_ilmu_id
                WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
    ...
    {
        $row = $this->show($id); // simpan data sebelum dihapus

        $stmt = $this->pdo->prepare("DELETE FROM penelitian WHERE id = :id");
        $stmt->execute(['id' => $id]);
